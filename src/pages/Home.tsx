...
const USE_CASES = [
  ...
  { icon: Cloud,         title: 'Service récurrent ou SaaS',        desc: 'CRM, outil de gestion, abonnement logiciel.' },
  ...
  { icon: Boxes,         title: 'Projet avec plusieurs financeurs', desc: 'Subside, fonds européen, co-financement public.' },
...
const FAQS = [
  ...
  { q: 'Comment interpréter mon résultat ?', a: "Le résultat est une orientation indicative — pas un avis juridique. Il vous indique un niveau de vigilance (faible, modéré, élevé) et les points à vérifier avant d'engager votre dépense. En cas de doute sérieux, consultez une personne compétente." },
  { q: 'Quand dois-je consulter un juriste ou un expert ?', a: "Dès que votre niveau de vigilance est modéré ou élevé, que votre convention de subvention contient des clauses d'achat, ou que le montant en jeu dépasse 30 000 € HTVA. Un juriste ou un service compétent en marchés publics peut sécuriser votre procédure." },
]
...
const TRUST_ITEMS: TrustItem[] = [
  ...
  { Icon: Check, label: 'Résultat indicatif' },
]
    ...
    <header aria-label="Navigation principale" className={`sticky top-0 z-50 transition-all duration-200 ${scrolled ? 'bg-white border-b border-line shadow-[0_1px_8px_rgba(8,43,76,0.06)]' : 'bg-transparent'}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onStart} className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-coral text-white font-semibold text-sm hover:brightness-105 transition-all shadow-coral active:scale-[0.98]">
            Faire le test gratuit <ArrowRight className="w-4 h-4" />
          </button>
      ...
      {open && (
        <div className="md:hidden border-t border-line bg-white px-4 py-3 space-y-1">
          ...
          <button onClick={onStart} className="w-full mt-2 py-3 rounded-lg bg-coral text-white font-semibold text-sm">Faire le test gratuit</button>
        </div>
      )}
    </header>
  )
    ...
    <section id="top" className="relative overflow-hidden bg-navy text-white">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-16 pb-20 sm:pt-24 sm:pb-28">
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <p className="mt-5 text-lg text-aqua/80 leading-relaxed max-w-xl">
              Répondez à quelques questions sur votre structure, votre financement et votre projet pour obtenir une <strong className="text-white font-semibold">orientation indicative</strong> et les points à vérifier avant d'acheter ou de lancer votre commande.
            </p>
          ...
          <motion.div className="relative" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <div className="relative animate-floaty">
              ...
              <div className="absolute -bottom-5 -left-5 bg-navy text-white rounded-xl shadow-float px-4 py-3 flex items-center gap-2.5 animate-floaty-slow border border-white/10">
                ...
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-aqua/60 font-semibold">Orientation indicative</p>
        ...
        <div className="mt-16 pt-8 border-t border-white/10 grid grid-cols-3 gap-4 sm:gap-8 max-w-2xl">
          {([['5 min', 'Pour faire votre premier tri'], ['8 questions', 'Guidées, sans jargon'], ['100 %', 'Indépendant & gratuit']] as [string, string][]).map(([n, l]) => (
            <div key={n}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-16 items-center mb-16">
          <Reveal className="max-w-2xl">
            ...
            <h2 className="mt-4 font-display text-3xl sm:text-4xl font-bold text-navy tracking-tight text-balance">Beaucoup d’associations achètent sans savoir ce qu’il faut vérifier.</h2>
            <p className="mt-4 text-slate leading-relaxed">marchépublic.be vous aide à faire un premier tri avant d’acheter. En répondant à quelques questions sur votre structure, votre financement et votre projet, vous obtenez une orientation indicative et les points de vigilance à vérifier — sans jargon juridique.</p>
          </Reveal>
...
function HowSection({ onStart }: { onStart: () => void }) {
  const steps = [
    ...
    { n: '03', icon: Gauge,   title: "Obtenir votre orientation", desc: "Vous recevez une orientation indicative, un chemin recommandé et les sources officielles à vérifier." },
  ]
  return (
    <section id="comment" className="relative bg-ink text-white py-20 sm:py-28 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        <Reveal className="max-w-xl mb-14">
          ...
          <h2 className="mt-4 font-display text-3xl sm:text-4xl font-bold tracking-tight text-balance">Un parcours structuré en trois temps</h2>
          <p className="mt-3 text-aqua/75">Le parcours suit une logique progressive : structure, projet, montant. À chaque réponse, l’orientation se précise.</p>
        </Reveal>
        ...
        <Reveal className="mt-12 grid sm:grid-cols-[1.2fr_0.8fr] gap-8 items-center bg-white/[0.04] border border-white/10 rounded-2xl p-6 sm:p-8">
          ...
          <div className="text-center sm:text-left">
            ...
            <h3 className="font-display font-bold text-lg mb-2">Une orientation, pas un verdict</h3>
            <p className="text-aqua/70 text-sm leading-relaxed">À la fin, vous obtenez une orientation indicative, le chemin recommandé, des documents utiles et les sources officielles à vérifier.</p>
          </div>
        </Reveal>
...
function ObtainSection({ onStart }: { onStart: () => void }) {
  const outcomes = [
    { icon: Gauge,         title: 'Orientation indicative',       desc: 'Un niveau de vigilance (faible, modéré, élevé) qui reflète les éléments connus de votre situation.' },
    { icon: Target,        title: 'Ce qui influence le résultat', desc: 'Les critères qui ont pesé dans votre score, expliqués simplement.' },
    { icon: MapPin,        title: 'Chemin recommandé',            desc: 'Les étapes concrètes à envisager selon votre niveau de vigilance.' },
    { icon: ClipboardList, title: 'Documents utiles',             desc: "Des fiches pratiques pour structurer votre démarche d'achat." },
    { icon: ShieldCheck,   title: 'Sources officielles',          desc: 'Les liens vers les textes de référence belges pour confirmer la procédure applicable.' },
    { icon: Compass,       title: 'Pistes si vous avez un doute', desc: 'Des ressources et points de contact pour aller plus loin si votre situation reste incertaine.' },
  ]
  return (
    <section id="obtenir" className="bg-white py-20 sm:py-28">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <Reveal className="max-w-2xl mb-10">
          <Eyebrow>Ce que vous obtenez</Eyebrow>
          <h2 className="mt-3 font-display text-3xl sm:text-4xl font-bold text-navy tracking-tight text-balance">Un résultat concret, pas une réponse floue</h2>
          <p className="mt-3 text-slate">À la fin du parcours, vous repartez avec des éléments actionnables pour avancer avec méthode.</p>
        </Reveal>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {outcomes.map((o, i) => (
            <Reveal key={o.title} delay={i * 0.06}>
              <div className="h-full bg-cream rounded-2xl border border-line p-6 shadow-card">
                <div className="w-10 h-10 rounded-xl bg-navy flex items-center justify-center mb-4">
                  <o.icon className="w-5 h-5 text-teal" />
                </div>
                <h3 className="font-display font-bold text-navy mb-1.5">{o.title}</h3>
                <p className="text-slate text-sm leading-relaxed">{o.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
        <Reveal className="mt-8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <p className="text-xs text-slate/70">Tout est gratuit, sans inscription. Vos réponses restent sur votre appareil.</p>
        </Reveal>
      </div>
    </section>
  )
...
function ClarifySection() {
  const can = [
    ...
    'Un niveau de vigilance indicatif pour orienter votre démarche',
  ]
  const cannot = [
    ...
    'Tenir compte de vos conventions de subvention spécifiques',
  ]
  return (
    <section className="bg-cream py-20 sm:py-28">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <Reveal className="mb-10">
          ...
          <h2 className="mt-3 font-display text-3xl font-bold text-navy tracking-tight">Ce que le test peut faire — et ce qu’il ne fait pas</h2>
        </Reveal>
        <div className="grid lg:grid-cols-2 gap-5">
          <Reveal>
            <div className="h-full bg-white rounded-2xl border border-line p-7 shadow-card">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h3 className="font-display font-bold text-navy text-lg">Ce que le test peut clarifier</h3>
              </div>
          ...
          <Reveal delay={0.08}>
            <div className="h-full bg-navy text-white rounded-2xl p-7 shadow-card relative overflow-hidden">
              ...
              <p className="mt-5 text-xs text-aqua/60 leading-relaxed border-t border-white/10 pt-4">Ce test fournit une orientation indicative basée sur vos réponses. Il ne constitue pas un avis juridique. En cas de doute, consultez une personne compétente avant d’engager votre dépense.</p>
            </div>
          </Reveal>
        </div>
      </div>
    ...
    <section id="cas" className="bg-white py-20 sm:py-28">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <Reveal className="max-w-2xl mb-10">
          ...
          <p className="mt-3 text-slate">Ces achats du quotidien associatif déclenchent souvent la question. Cliquez sur le vôtre pour commencer le test.</p>
        </Reveal>
    ...
    <section className="relative bg-teal text-navy py-20 sm:py-24 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
        <Reveal>
          ...
          <p className="mt-3 text-navy/65 max-w-xl mx-auto">Si vous gérez une structure qui reçoit des fonds publics, collabore avec des organismes publics, ou s’interroge sur ses réflexes avant d’acheter, ce test est fait pour vous.</p>
        </Reveal>
            ...
            <div className="relative px-6 py-10 sm:px-10 sm:py-12">
              <div className="mb-8">
                ...
                <h2 className="font-display text-2xl sm:text-3xl font-bold leading-tight max-w-lg">
                  marchépublic.be aide à clarifier ce qu’il faut vérifier.<br />
                ...
                <p className="mt-3 text-aqua/70 text-sm leading-relaxed max-w-md">
                  Une fois les points de vigilance identifiés, passer à l’action demande méthode et documentation. Nomad Impact accompagne les ASBL et organisations belges dans la définition et la mise en œuvre de leurs projets.
                </p>
              </div>
              ...
              <p className="mt-4 text-xs text-aqua/50">Besoin d’un accompagnement pour y voir clair ? Nomad Impact peut vous aider à documenter votre décision.</p>
            </div>
          </div>
        </Reveal>
      </div>
        ...
        <Reveal>
          <div className="relative overflow-hidden rounded-2xl bg-navy text-white px-6 py-14 sm:px-12 sm:py-16 text-center border border-white/5 shadow-float">
            ...
            <div className="relative">
              <h2 className="font-display text-3xl sm:text-4xl font-bold tracking-tight text-balance max-w-2xl mx-auto">Un projet en tête ? Faites d’abord le point.</h2>
              <p className="mt-4 text-aqua/70 max-w-xl mx-auto text-sm leading-relaxed">Cinq minutes pour obtenir une orientation indicative, un chemin recommandé et les sources officielles belges à vérifier.</p>
    ...
    <footer className="bg-ink text-aqua/50 border-t border-white/5">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
          <div className="lg:col-span-1">
            ...
            <button onClick={onStart} className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-teal hover:text-white transition-colors">
              Faire le test gratuit <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
          <div>
            <p className="text-white text-[10px] font-bold uppercase tracking-widest mb-4">Naviguer</p>
            <ul className="space-y-2.5 text-sm">
              {([['#pourquoi', 'Pourquoi cet outil'], ['#comment', 'Comment ça marche'], ['#obtenir', 'Ce que vous obtenez'], ['#cas', 'Cas fréquents'], ['#faq', 'FAQ']] as [string, string][]).map(([h, l]) => (
        ...
        <div className="border-t border-white/8 pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <p className="text-[11px]">Une initiative de <a href="https://nomadimpact.org" target="_blank" rel="noopener noreferrer" className="text-white font-semibold hover:text-teal transition-colors">Nomad Impact ASBL</a></p>
        </div>
      </div>
    </footer>
  )
